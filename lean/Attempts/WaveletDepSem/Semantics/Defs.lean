...
structure Semantics (S : Type u) (Op : Type v) (V : Type w) [Arity Op] m n : Type (max u v w) where
  init : S
  lts : Lts S (Label Op V m n)

@[simp]
abbrev Semantics.State [Arity Op] (_sem : Semantics S Op V m n) := S
...
instance [Arity Op] : IsRefl (Semantics S Op V m n) IORestrictedSimilarity where
  refl := .refl

instance [Arity Op] : IsTrans (Semantics S Op V m n) IORestrictedSimilarity where
  trans _ _ _ := .trans
...
def IsInvariantAt
  ...
  (sem : Semantics S Op V m n)
  (P : S → Prop)
  (s : S) : Prop := sem.lts.IsInvariantAt P s

def IsInvariant
  [Arity Op]
  (sem : Semantics S Op V m n)
  (P : S → Prop) : Prop := sem.IsInvariantAt P sem.init
...
def IsFinalFor
  ...
  (s : S) : Prop := sem.lts.IsFinalFor Labels s
